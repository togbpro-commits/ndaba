...
4. **Absolute Positioning Overflow in Non-Positioned Containers:** Absolute elements (like `w-[140%]` background glows) placed inside root containers that have `overflow-x-hidden` but lack `relative` positioning will escape clipping because their containing block resolves to the viewport. Adding `relative` strictly binds and clips them, preventing mobile horizontal overscroll.
5. **Inngest SDK vs CLI Binaries:** Running `npx inngest dev` fails if the local `inngest` package only represents the library SDK, because it lacks executable scripts. The proper package for CLI utilities is `inngest-cli` (invoked via `npx inngest-cli@latest dev`).
6. **Accordion Layout Shifts:** Rendering expanding elements instantly causes rigid layout jumps. Categorizing items into tabs to keep lists small, combined with Framer Motion height-auto transition wrappers and parent min-height bounds, ensures absolute visual stability on click, preventing page jarring.
